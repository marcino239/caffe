...
/**
 * @brief Computes the average distance for a position estimation
 *        task.
 */
template <typename Dtype>
class EuclideanDistanceLayer : public Layer<Dtype> {
 public:
  explicit EuclideanDistanceLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "EuclideanDistance"; }
 ...
 *   -# @f$ (N \times C \times 1 \times 1) @f$
 *      the targets @f$ y \in [-\infty, +\infty]@f$
 *   -# @f$ (N \times C \times C \times 1) @f$
 *      the weight matrix
 * @param top output Blob vector (length 1)
 *   -# @f$ (1 \times 1 \times 1 \times 1) @f$
 *      the computed Weighted Euclidean loss: @f$ E =
 *          \frac{1}{2n} \sum\limits_{n=1}^N \left| \left| \hat{y}_n - y_n
 *        \right| \right|_2^2 @f$
 *
 * This can be used for least-squares regression tasks.  An InnerProductLayer
...
/**
 * @brief Computes the Max Likelihood loss for an predicted mean and variance
 *
 * @param bottom input Blob vector (length 2)
 *   -# @f$ (N \times C \times 1 \times 1) @f$
 *      the predictions @f$ \hat{y} \in [-\infty, +\infty]@f$
 ...
 */
template <typename Dtype>
class MLLossLayer : public LossLayer<Dtype> {
 public:
  explicit MLLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

	// MLLossLayer takes 2 Blobs. The first should be the predicted mean and
	// variance, the second should be the target mean
  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline const char* type() const { return "MLLoss"; }
 ...
 protected:
  ...
  /**
   * @brief Computes the ML error gradient w.r.t. the inputs.
   *
   * @param top output Blob vector (length 1), providing the error gradient with
   *      respect to the outputs
   *   -# @f$ (1 \times 1 \times 1 \times 1) @f$
